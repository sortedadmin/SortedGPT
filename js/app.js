...
/* Browser back/Android back-gesture is handled by the single navigation controller below.
   Do not force every popstate to the homepage. */
...
/* external: https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2 */
...
window.openBusinessManagement=function(id){
  ...
  // Android back gesture: business detail -> business profile list, without leaving the seller screen.
  window.addEventListener('popstate',function(e){
    if(window.__businessManagementSelected){
      window.__businessManagementSelected=false;
      try{history.replaceState({sellerBusinesses:true},'',location.pathname+'#seller-businesses');}catch(err){}
      renderBusinessManagementList();
...
(function(){
  function escV122(v){return typeof esc==='function'?esc(v):String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
  function sellerV122(){const u=typeof user==='function'?user():null;return u&&String(u.role).toLowerCase()==='seller'?u:null;}
  function ownedV122(){const u=sellerV122();return u?(db.businesses||[]).filter(b=>b.ownerId===u.id):[];}
  function renderV122(){
    const root=document.getElementById('sellerContent');
    const u=sellerV122();
    if(!root||!u)return;
    ...
    const bs=ownedV122();
    if(!bs.length){
      root.innerHTML='<div class="selected-business-wrap"><div class="page-head"><div class="page-title">BUSINESSES</div></div><button class="btn primary full" onclick="createBusiness()">+ ADD NEW BUSINESS</button><div class="empty"><strong>No businesses yet</strong><div style="margin-top:7px">Create your first business to start adding products.</div></div></div>';
      return;
    }
    if(!currentBusiness||!bs.some(b=>String(b.id)===String(currentBusiness)))currentBusiness=bs[0].id;
    const b=bs.find(x=>String(x.id)===String(currentBusiness))||bs[0];
    currentBusiness=b.id;
    try{sellerSelectedBusiness=currentBusiness}catch(e){}
    const products=(db.products||[]).filter(p=>String(p.businessId)===String(b.id));
    const cat=(db.categories||[]).find(c=>String(c.id)===String(b.categoryId))?.name||b.category||'Local Business';
    root.innerHTML=`
      <div class="selected-business-wrap">
        <div class="page-head"><div class="page-title">BUSINESSES</div></div>
        <button class="btn primary full selected-add-business" onclick="createBusiness()">+ ADD NEW BUSINESS</button>
        <div class="selected-business-picker">
          <label>SELECT BUSINESS TO MANAGE</label>
          <select onchange="window.__v122SelectBusiness(this.value)">${bs.map(x=>`<option value="${escV122(x.id)}" ${String(x.id)===String(b.id)?'selected':''}>${escV122(x.name)}</option>`).join('')}</select>
          <div>Only the selected business is shown below for editing, products and deletion.</div>
        </div>
        <div class="selected-business-card">
          <div class="selected-business-cover">${b.cover?`<img src="${escV122(b.cover)}" alt="">`:'<div style="height:100%;display:grid;place-items:center;color:#7b8781;font-size:11px">NO COVER IMAGE</div>'}</div>
          <div class="selected-business-main">
            <div class="selected-business-head">
              ${b.logo?`<img class="selected-business-logo" src="${escV122(b.logo)}" alt="">`:'<div class="selected-business-logo" style="display:grid;place-items:center;font-size:24px">◎</div>'}
              <div><div class="selected-business-name">${escV122(b.name)}</div><div class="selected-business-meta">${escV122(cat)} · 📍 ${escV122(b.locality||b.address||'Local business')}</div><div class="selected-business-meta">${products.length} product${products.length===1?'':'s'} · ${b.delivery?'Delivery available':'Pickup only'}</div></div>
            </div>
            <div class="selected-business-actions"><button class="btn primary" onclick="editBusinessProfile('${escV122(b.id)}')">EDIT BUSINESS</button><button class="btn danger" onclick="deleteSelectedBusinessV81()">DELETE BUSINESS</button></div>
          </div>
        </div>
        <div class="selected-products-head"><div><h3>PRODUCTS</h3><div class="selected-products-count">Products belonging only to ${escV122(b.name)}</div></div><button class="btn primary" onclick="addProduct('${escV122(b.id)}')">+ ADD PRODUCT</button></div>
        <div>${products.length?products.map(p=>{const img=p.image||(Array.isArray(p.images)?p.images[0]:'');return `<div class="selected-product"><div class="selected-product-img">${img?`<img src="${escV122(img)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:12px" loading="lazy" onerror="this.style.display='none'">`:'▣'}</div><div class="selected-product-info"><b>${escV122(p.name)}</b><span>₹${Number(p.discountPrice||p.price||0).toLocaleString('en-IN')} · ${Number(p.stock||0)} ${escV122(p.unit||'piece')} in stock</span></div><div class="selected-product-actions"><button class="btn secondary" onclick="editProduct('${escV122(p.id)}')">EDIT</button><button class="btn secondary" onclick="changeProductImage('${escV122(p.id)}')">PHOTOS</button><button class="btn danger" onclick="deleteSelectedProductV81('${escV122(p.id)}')">DELETE</button></div></div>`}).join(''):`<div class="selected-empty">No products in <b>${escV122(b.name)}</b> yet.<br><button class="btn primary" style="margin-top:10px" onclick="addProduct('${escV122(b.id)}')">+ ADD FIRST PRODUCT</button></div>`}</div>
      </div>`;
  }
  window.__v122SelectBusiness=function(id){const bs=ownedV122();if(!bs.some(b=>String(b.id)===String(id)))return;currentBusiness=id;try{sellerSelectedBusiness=id}catch(e){}renderV122();};
  window.renderSellerBusinesses=renderV122;
  window.renderSelectedBusinessScreen=renderV122;
  const oldSellerTabV122=window.sellerTab;
  window.sellerTab=function(tab){
    if(tab==='businesses'){
      const u=sellerV122();if(!u)return;
      document.body.classList.add('seller-mode');
      document.querySelectorAll('.buyer-nav').forEach(n=>n.style.display='none');
      const sn=document.getElementById('sellerNav');if(sn)sn.style.display='grid';
      document.querySelectorAll('.screen').forEach(x=>x.classList.remove('active'));
      const ss=document.getElementById('seller');if(ss)ss.classList.add('active');
      document.querySelectorAll('#sellerNav button').forEach(x=>x.classList.toggle('active',x.dataset.sellerTab==='businesses'));
      try{history.replaceState({screen:'seller'},'',location.pathname+'#seller')}catch(e){}
      renderV122();try{scrollTo({top:0,behavior:'auto'})}catch(e){}
      return;
    }
    if(typeof oldSellerTabV122==='function')return oldSellerTabV122.apply(this,arguments);
  };
  window.renderSellerTab=function(){
    const active=document.querySelector('#sellerNav button.active')?.dataset.sellerTab;
    if(active==='businesses')return renderV122();
    if(typeof window.renderSellerDashboard==='function'&&active==='dashboard')return window.renderSellerDashboard();
    if(typeof window.renderSellerReservations==='function'&&active==='reservations')return window.renderSellerReservations();
    if(typeof window.renderSellerProfile==='function'&&active==='profile')return window.renderSellerProfile();
  };
})();
